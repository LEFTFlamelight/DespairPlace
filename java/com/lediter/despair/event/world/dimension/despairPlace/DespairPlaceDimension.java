...
package com.lediter.despair.event.world.dimension.despairPlace;
...
public class DespairPlaceDimension {
	@ObjectHolder("despair:despair_place_portal")
	public static final CustomPortalBlock portal = BlockRegistry.PORTAL_BLOCK.get();
	private static PointOfInterestType poi = null;
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("despair_place_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("despair_place_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("despair_place_portal");
			ForgeRegistries.POI_TYPES.register(poi);
		}
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			super(Properties.create(Material.PORTAL).doesNotBlockMovement().tickRandomly().hardnessAndResistance(-1.0F).sound(SoundType.GLASS)
		...
		public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos,
			...
			return !flag && !facingState.matchesBlock(this) && !(new CustomPortalSize(worldIn, currentPos, direction$axis1)).validatePortal()
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) Objects.requireNonNull(ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("block.portal.ambient")))),
						SoundCategory.BLOCKS, 0.5f, random.nextFloat() * 0.4F + 0.8F, false);
		}

		@Override
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.canChangeDimension() && !entity.world.isRemote) {
				if (entity.hasPortalCooldown()) {
					...
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("despair:despair_place")));
				} else {
					entity.setPortalCooldown();
		...
		private void teleportToDimension(Entity entity, BlockPos pos, RegistryKey<World> destinationType) {
			entity.changeDimension(Objects.requireNonNull(Objects.requireNonNull(entity.getServer()).getWorld(destinationType)),
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == BlockRegistry.DESPAIR_STONE.get();
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? BlockRegistry.DESPAIR_STONE.get().getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, BlockRegistry.DESPAIR_STONE.get().getDefaultState(), 3);
					}
				}
			}
